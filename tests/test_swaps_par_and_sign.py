...
from insurance_hedging_simulator.hedge_swap import (
    par_swap_rate, swap_pv_payer_fixed, SizedSwap
)
...
def test_par_swap_has_zero_pv_and_payer_fixed_has_negative_dv01():
    ...
    pv = swap_pv_payer_fixed(curve, notional=1.0, maturity_years=maty, payments_per_year=1, fixed_rate=k)
    assert abs(pv) < 1e-8

    # SizedSwap with pay_fixed=True should have negative DV01 (value rises when rates rise)
    s = SizedSwap(maturity_years=maty, payments_per_year=1, pay_fixed=True, notional=1.0, fixed_rate=k)
